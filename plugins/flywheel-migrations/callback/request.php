	...
	class BVCallbackRequest {
		...
		public function __construct($account, $in_params, $settings) {
			...
			$this->pubkey_name = !empty($in_params['pubkeyname']) ?
					FWLAccount::sanitizeKey($in_params['pubkeyname']) : 'm_public';
			$this->bvprmsmac = !empty($in_params['bvprmsmac']) ? FWLAccount::sanitizeKey($in_params['bvprmsmac']) : "";
		...
		public function authFailedResp() {
			$api_public_key = FWLAccount::getApiPublicKey($this->settings);
			$default_secret = FWLRecover::getDefaultSecret($this->settings);
			$bvinfo = new FWLInfo($this->settings);
			$resp = array(
				"request_info" => $this->info(),
				"bvinfo" => $bvinfo->info(),
