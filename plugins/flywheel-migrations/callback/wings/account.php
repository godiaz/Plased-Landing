...
class BVAccountCallback extends BVCallbackBase {
	...
	function updateInfo($args) {
		...
		if (array_key_exists('update_info', $args)) {
			...
			$result['update_info'] = array(
				"status" => FWLAccount::exists($this->settings, $args['update_info']['pubkey'])
			);
		}
	
		if (array_key_exists('update_api_key', $args)) {
			FWLAccount::updateApiPublicKey($this->settings, $args['update_api_key']['pubkey']);
			$result['update_api_key'] = array(
				"status" => $this->settings->getOption(FWLAccount::$api_public_key)
			);
		}
	...
	function process($request) {
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = FWLAccount::allAccounts($this->settings);
